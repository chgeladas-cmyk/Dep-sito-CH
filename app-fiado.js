...
const FiadoService = (() => {
  ...
  function registrarVendaFiado(clienteId, valor, descricao = '', itens = []) {
    ...
    Store.mutate(state => {
      ...
      if (idx !== -1) {
        ...
        cli.historico.unshift({
          ...
          itens:     Array.isArray(itens) ? itens.map(i => ({ nome: i.nome, qtd: i.qtd || i.desconto || 1, preco: i.preco })) : [],
  ...
  function _processarVendaFiado(clienteId, valorVenda) {
    ...
    if (!venda) return;
    registrarVendaFiado(clienteId, valorVenda, `Venda #${String(venda.id).slice(-6)}`, venda.itens || []);
    ...
    if (lEl) {
      if (!hist.length) {
        ...
        lEl.innerHTML = hist.map(h => {
          const itensHtml = (h.tipo === 'DEBITO' && Array.isArray(h.itens) && h.itens.length)
            ? `<div class="mt-1.5 pt-1.5 border-t border-red-500/15 space-y-0.5">
                ${h.itens.map(i => `<p class="text-[8px] text-slate-500 font-bold flex justify-between gap-2"><span>• ${_esc(i.nome)} x${i.qtd ?? 1}</span><span class="text-slate-600">${Utils.formatCurrency(i.preco)}</span></p>`).join('')}
               </div>`
            : '';
          return `
          <div class="px-3 py-2.5 rounded-xl ${h.tipo === 'CREDITO'
