...
const EstoqueService = (() => {
      ...
      div.innerHTML = `
        <article class="glass-card rounded-2xl p-4 ${esgotado ? 'opacity-60' : ''}" aria-label="${RenderService._escapeHtml(p.nome)}">
          ...
          <div class="flex gap-2" role="group" aria-label="Ações de ${RenderService._escapeHtml(p.nome)}">
            ...
            <button onclick="registrarAvaria('${p.id}')" class="py-2 px-3 rounded-lg bg-orange-500/10 text-orange-400 hover:bg-orange-500 hover:text-white text-[8px] font-black uppercase transition-all" aria-label="Registrar avaria de ${RenderService._escapeHtml(p.nome)}" title="Baixar avaria/perda"><i class="fas fa-box-open" aria-hidden="true"></i></button>
  ...
  async function registrarAvaria(prodId) {
    const prod = Store.Selectors.getProdutoById(prodId);
    if (!prod) return;
    if (prod.qtdUn <= 0) {
      UIService.showToast('Avaria', `${prod.nome} já está sem estoque`, 'warning');
      return;
    }
    const qtdStr = await Dialog.prompt({
      title:        `Baixar Avaria — ${prod.nome}`,
      message:      `Estoque atual: ${prod.qtdUn} un.\nQuantas unidades vieram estragadas?`,
      placeholder:  'Ex: 2',
    ...
    if (!qtdStr) return;
    const qtd = parseInt(String(qtdStr).replace(',', '.')) || 0;
    if (qtd <= 0) { UIService.showToast('Avaria', 'Quantidade inválida', 'error'); return; }
    if (qtd > prod.qtdUn) {
      UIService.showToast('Avaria', `Máximo disponível: ${prod.qtdUn} un`, 'error');
      return;
    }
    const qtdDepois = prod.qtdUn - qtd;
    Store.mutate(state => {
      const p = state.estoque.find(x => String(x.id) === String(prodId));
      if (p) p.qtdUn = qtdDepois;
    }, true);
    _registrarAuditEstoque({
      prodId, produto: prod.nome,
      qtdAntes: prod.qtdUn, qtdDepois,
      motivo: `Avaria/Mercadoria estragada (${qtd} un)`,
      responsavel: 'Admin',
      tipo: 'AVARIA',
    });
    SyncService.persist();
    EventBus.emit('estoque:updated');
    UIService.showToast('Avaria registrada', `${qtd} un de "${prod.nome}" baixadas`, 'warning');
  }

  return Object.freeze({
    getTempPacks, getEpPacks, isEditing,
    resetForm, adicionarPack, removerTempPack, salvarProduto,
    editarProduto, removerProduto, registrarAvaria,
...
function registrarAvaria(id)     { EstoqueService.registrarAvaria(id); }
...
function renderAuditEstoque() {
  ...
  logs.slice(0, 120).forEach(l => {
    ...
    const isAvaria  = l.tipo === 'AVARIA';
    const borderCls = isAvaria  ? 'border-orange-500/15 bg-orange-500/5'
                    : isEntrada ? 'border-emerald-500/15 bg-emerald-500/5'
                                : 'border-red-500/15 bg-red-500/5';
    const iconBg    = isAvaria  ? 'bg-orange-500/20'  : isEntrada ? 'bg-emerald-500/20' : 'bg-red-500/20';
    const iconCls   = isAvaria  ? 'fa-box-open text-orange-400' : isEntrada ? 'fa-arrow-up text-emerald-400' : 'fa-arrow-down text-red-400';
    const deltaCls  = isAvaria  ? 'text-orange-400'   : isEntrada ? 'text-emerald-400' : 'text-red-400';
    const tipoBadge = isAvaria  ? '<span class="inline-block text-[7px] font-black uppercase bg-orange-500/15 text-orange-400 border border-orange-500/20 px-1.5 py-0.5 rounded-full ml-1">Avaria</span>' : '';
    const div = document.createElement('div');
    div.innerHTML = `
      <div class="flex items-start gap-3 px-3 py-2.5 rounded-xl border ${borderCls}">
        <div class="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${iconBg}">
          <i class="fas ${iconCls} text-[9px]"></i>
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-[10px] font-black text-slate-200 truncate">${l.produto || '—'}${tipoBadge}</p>
        ...
        <div class="text-right flex-shrink-0">
          <p class="text-[11px] font-black ${deltaCls}">${isEntrada ? '+' : ''}${l.delta} un</p>
